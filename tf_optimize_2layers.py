"""
Two layers for optimization. Only reason for using this is inability to reset graph under "while optimize nest"
"""
...
# IMG_PATCH_SIZE should be a multiple of 4 (since image is 400x400)
...
def main(t_size,filter_size,strides,num_filters, patch_size, num_epochs, mom = 0.0, base_rate = 0.01, k_fold_index = None, k_fold = None): 
    ...
    def get_prediction_test(filename, image_idx):
        ...
        image_filename = filename + imageid1 + imageid2 + ".png"
        img = mpimg.imread(image_filename)

        img_prediction = get_prediction(img)
        img_prediction = 1*numpy.logical_not(img_prediction) #Otherwise road's will be black and non-roads white
